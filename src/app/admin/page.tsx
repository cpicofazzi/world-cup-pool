...
export default async function AdminMatchesPage() {
  ...
  // Fetch team count to show sync status
      ...
      <div className="bg-brand-navy border-b border-brand-green/30 pt-10 pb-16 relative overflow-hidden">
        <div className="max-w-6xl mx-auto text-center relative z-10 px-4">
          ...
          <p className="text-slate-400 text-sm font-mono uppercase tracking-widest">
             Sync data, enter scores, manage the tournament
          </p>
          <form action={async (formData) => {
            'use server'
            await triggerDataSync(formData)
          }} className="mt-6">
            <button type="submit" className="bg-brand-green text-brand-navy px-8 py-3 rounded-full text-sm font-bold hover:bg-brand-green/90 shadow-sm transition-colors uppercase tracking-wider">
              ⚡ Sync Tournament Data
            </button>
          </form>
          <div className="mt-4 flex justify-center gap-6">
            <span className="text-xs text-slate-400 font-mono">
              Teams in DB: <span className="text-brand-gold font-bold">{teamCount ?? 0}</span>
            </span>
            <span className="text-xs text-slate-400 font-mono">
              Matches in DB: <span className="text-brand-gold font-bold">{matches?.length ?? 0}</span>
            </span>
          </div>
        </div>
      </div>
      ...
      <main className="max-w-6xl mx-auto px-4 md:px-0 -mt-8 relative z-20 pb-20">
        ...
        {(!matches || matches.length === 0) ? (
          <div className="bg-surface border border-border-subtle rounded-xl p-12 text-center shadow-sm">
            ...
            <h2 className="text-xl font-bold text-brand-navy mb-2">No Matches Yet</h2>
            <p className="text-slate-500 text-sm max-w-md mx-auto mb-6">
              {teamCount && teamCount > 0 
                ? `${teamCount} teams synced! Match schedule may not be published yet on Football-Data.org. Click "Sync Tournament Data" again once the schedule is available.`
                : 'Click "Sync Tournament Data" above to pull teams and match schedule from Football-Data.org.'
              }
            </p>
          </div>
        ) : (
